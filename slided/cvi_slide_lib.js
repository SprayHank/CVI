/**
 * cvi_slide_lib.js 1.5 (10-Aug-2010) (c) by Christian Effenberger 
 * All Rights Reserved. Source: slided.netzgesta.de
...
var cvi_slide = {
	...
	add: function(image, options) {
		if(image.tagName.toUpperCase() == "IMG") {
			var defopts = { "shade" : cvi_slide.defaultShade, "shadow" : cvi_slide.defaultShadow, "color" : cvi_slide.defaultColor, "color2" : cvi_slide.defaultColor2, "gradient" : cvi_slide.defaultGradient, "noshadow" : cvi_slide.defaultNoshadow, "nocircles" : cvi_slide.defaultNocircles, "textcolor" : cvi_slide.defaultTextcolor, "fontattr": cvi_slide.defaultFontattr, "text" : cvi_slide.defaultText, "filter" : cvi_slide.defaultFilter, "callback" : cvi_slide.defaultCallback }
			if(options) {
		...
		try {
			...
			if(document.all&&document.namespaces&&!window.opera&&(!document.documentMode||document.documentMode<9)) {
				if(canvas.tagName.toUpperCase() == "VAR") {
					...
					if(typeof window[callback]==='function') {window[callback](canvas.id,'cvi_slide');}
				}
			}else {
